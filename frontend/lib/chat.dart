...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _loadHistoryAndConnect() async {
    ...
    // 3. Connect WebSocket
    await _connectWebSocket(sessionId: sessionId);
    setState(() => _isTyping = false);
  }

  Future<void> _connectWebSocket({int? sessionId}) async {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('access_token');
    if (token == null) return;

    final wsUrl = "ws://localhost:8001/ws/chat?token=$token${sessionId != null ? '&session_id=$sessionId' : ''}";
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            roadmap['summary'] ?? "Here is your plan:",
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          ...
                          style: const TextStyle(color: _pink, fontWeight: FontWeight.w800, fontSize: 11, letterSpacing: 0.5),
                        ),
                      ),
                    ],
                  ),
